...
("`@startyaml`n" + $mysub.toYAML() + "`n`@endyaml") | Out-File -FilePath (Join-Path $ScriptPath 'plantuml' '#0 init 192.168.0.0 from 29 to 31.puml')
("`@startjson`n" + $mysub.toJSON() + "`n`@endjson") | Out-File -FilePath (Join-Path $ScriptPath 'plantuml' '#0 init 192.168.0.0 from 29 to 31 (2).puml')
("`@startuml`n" + $mysub.toDOT() + "`n`@enduml") | Out-File -FilePath (Join-Path $ScriptPath 'plantuml' '#0 init 192.168.0.0 from 29 to 31 (3).puml')
